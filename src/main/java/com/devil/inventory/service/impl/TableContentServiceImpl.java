...
import com.devil.inventory.model.dao.entity.CellInfoEntity;
import com.devil.inventory.model.dao.entity.ReferenceInfoEntity;
import com.devil.inventory.model.dao.entity.TableInfoEntity;
import com.devil.inventory.model.service.dto.TableContentDTO;
...
import com.devil.inventory.service.service.*;
import lombok.Data;
...
public class TableContentServiceImpl implements ITableContentService {
    ...
    private List<TableContentDTO> mergeTableInfos(List<TableInfoEntity> tableInfoEntityList, Map<Long, List<ReferenceInfoEntity>> referenceInfoEntityMap, Map<Long, List<CellInfoEntity>> cellInfoEntityMap) {
        ...
        List<ReferenceInfoEntity> rows = null;
        List<ReferenceInfoEntity> cols = null;
        Map<Long, TableContentDTO.ColEntry> colEntryMap = null;
        List<TableContentDTO.ColEntry> colEntryList = null;
        TableContentDTO.ColEntry colEntry = null;
        for (TableInfoEntity tableInfoEntity : tableInfoEntityList) {
            ...
            for (ReferenceInfoEntity referenceInfoEntity : referenceInfoEntityList) {
                ...
                if (referenceInfoEntity.getReferenceType() == COL_TYPE) {
            ...
            //set colEntry
            colEntryMap = new HashMap<>();
            for (CellInfoEntity cellInfoEntity : cellInfoEntityList) {
                if (cellInfoEntity == null) {
                    continue;
                }
                long cellId = cellInfoEntity.getId();
                long colId = cellInfoEntity.getColId();
                colEntry = colEntryMap.get(colId);
                if (colEntry == null) {
                    colEntry = new TableContentDTO.ColEntry();
                    colEntryMap.put(colId, colEntry);
                }
                colEntry.setColId(colId);
                colEntry.addCellId(cellId);
            }
            dto.setColEntryMap(colEntryMap);
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    private Map<Long, List<CellInfoEntity>> getCellInfos(List<TableInfoEntity> tableInfoEntityList, Map<Long, List<ReferenceInfoEntity>> referenceInfoEntityMap) throws Exception {
        ...
        for (TableInfoEntity entity : tableInfoEntityList) {
            ...
            //按照行取数据
            for (ReferenceInfoEntity referenceInfoEntity : referenceInfoEntityList) {
                if (referenceInfoEntity == null || referenceInfoEntity.getReferenceType() == ROW_TYPE) {
                    continue;
                }
                long referenceId = referenceInfoEntity.getId();
                List<CellInfoEntity> tmpCellInfoList = cellInfoService.queryCellInfoByColId(referenceId);
                System.out.println("tmpCellInfoList : " + JSONObject.toJSONString(tmpCellInfoList) + " colId : " + referenceId);
